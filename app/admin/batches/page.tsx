...
export default function BatchesPage() {
	...
	const [formData, setFormData] = useState<BatchForm>(createDefaultForm);
	...
	const fetchData = useCallback(async () => {
		...
		try {
			...
			const batchList: Batch[] = Array.isArray(batchResult) ? batchResult : (batchResult?.data ?? []);
  ...
  `);
	...
	const handleOpenEdit = (batch: Batch) => {
		setFormData({
			...
			status: batch.status ? 'true' : 'false',
		});
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		if (endDate <= startDate) {
			setModalError('End date must be later than start date.');
			return;
		}
		...
		try {
			...
			setModalError(err instanceof Error ? err.message : 'Failed to save batch.');
		} finally {
			setSaving(false);
		}
	};
				...
				<main className={styles.mainContent}>
					<div className={styles.contentHeader}>
						<div>
							...
							<p className={styles.pageSubtitle}>Manage all registered batches.</p>
						</div>

						<div className={styles.headerActions}>
							<button type='button' className={styles.btnAdd} onClick={handleOpenCreate} disabled={loading}>
								Add New
							</button>
						</div>
					</div>
									...
									paginatedBatches.map((batch, index) => {
										return (
											<tr
												...
												<td>
													<span className={batch.status ? styles.statusActive : styles.statusDone}>
														<span className={styles.statusDot} />
														{batch.status ? 'Active' : 'Done'}
													</span>
												</td>
			...
			{isModalOpen && (
				<div className={styles.modalOverlay} onClick={handleCloseModal}>
					<div className={styles.modalContent} onClick={(event) => event.stopPropagation()}>
						<h2 className={styles.modalTitle}>{editingId ? 'Edit Batch' : 'Add New Batch'}</h2>
