...
export default function AdminPage() {
	...
	const isEditingOwnAccount = editingId !== null && editingId === currentUser?.id;
  ...
  `);
	...
	const handleDelete = async (id: number) => {
		const confirmed = await confirmAction('delete', 'this user');
				...
				<main className={styles.mainContent}>
					<div className={styles.contentHeader}>
						<div>
							...
							<p className={styles.pageSubtitle}>Manage all admins and teachers.</p>
						</div>

						<div className={styles.headerActions}>
							<button className={styles.btnAddUser} onClick={handleOpenCreate}>
								Add New User
							</button>
						</div>
					</div>
										...
										<tr
											...
											<td>
												<div className={styles.userCell}>
													<div className={styles.userInitials}>{getInitials(user.name)}</div>
											...
											<td>
												<span
													className={
														user.role === 'SUPER_ADMIN' ? styles.roleAdmin : styles.roleTeacher
													}>
													{user.role === 'SUPER_ADMIN' ? 'Admin' : 'Teacher'}
												</span>
											</td>

											<td className={styles.passwordText}>••••••••</td>

											<td>
												<span className={user.isActive ? styles.statusActive : styles.statusLeave}>
			...
			{isModalOpen && (
				<div className={styles.modalOverlay} onClick={() => setIsModalOpen(false)}>
					<div className={styles.modalContent} onClick={(event) => event.stopPropagation()}>
						<h2 className={styles.modalTitle}>{editingId ? 'Edit User' : 'Add New User'}</h2>
						...
						<form onSubmit={handleSubmit}>
							...
							<div className={styles.formGroup}>
								...
								<select
									name='role'
									value={isEditingOwnAccount ? currentUser?.role : formData.role}
									disabled={isEditingOwnAccount}
									onChange={handleChange}
									className={styles.inputField}>
									<option value='SUPER_ADMIN'>Admin</option>

									{!isEditingOwnAccount && <option value='TEACHER'>Teacher</option>}
								</select>
							</div>
							...
							<div className={styles.formGroup}>
								...
								<input
									...
									onChange={handleChange}
									className={styles.inputField}
									placeholder={editingId ? 'Leave empty to keep old password' : 'Set password'}
								/>
							</div>
